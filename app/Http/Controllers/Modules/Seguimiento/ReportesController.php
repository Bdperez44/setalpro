...
class ReportesController extends Controller {
	...
	public function getReportepractica(){
		...
		FROM 
			...
			sep_programa As pro, sep_nivel_formacion AS niv_for, sep_planeacion_ficha AS pla_fic
		WHERE 
	...
	public function getSelinstructor(){
		...
		WHERE 
			...
			AND pla_fic.pla_ins_lider = par.par_identificacion
			AND pro.niv_for_id = niv_for.niv_for_id 
			AND pla_fic.pla_ins_lider = '$vIngresado'";
	...
	public function getSelinstructortotal(){
		...
		SELECT 
			COUNT(*) AS total
		FROM 
			sep_ficha AS fic, sep_programa AS pro, sep_participante AS par, sep_nivel_formacion AS niv_for, sep_planeacion_ficha AS pla_fic
		WHERE 
		fic.prog_codigo = pro.prog_codigo AND
		pla_fic.pla_ins_lider = par.par_identificacion AND
		pla_fic.fic_numero = fic.fic_numero AND
		pro.niv_for_id = niv_for.niv_for_id AND
		pla_fic.pla_ins_lider = '$vIngresado'";
	...
	public function getReporteseguimiento(){
		...
		where productiva.ope_id = opcion.ope_id and not opcion.ope_id = 6");
