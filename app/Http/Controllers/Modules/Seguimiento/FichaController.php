...
class FichaController extends Controller {
    ...
    public function getIndex($valor= false, $filtro=false) {
        ...
        if($valor==""){
			...
			order by fic_numero asc limit '.$limit;

			$sqlContador = '
			select count(fic_numero) as total 
			from sep_ficha';

        }else{
            if($filtro == 1){
                $busqueda=" fic_numero like '%$valor%'";
            }elseif($filtro == 2){
                $busqueda=" prog_codigo like '%$valor%'";
            }elseif($filtro == 3){
                $busqueda=" fic_fecha_inicio like '%$valor%'";
			}elseif($filtro == 4){
                $busqueda=" fic_fecha_fin like '%$valor%'";
			}
			
		$sql = "select * from sep_ficha 
		where fic_numero=fic_numero and (".$busqueda.")
		order by fic_numero asc
		limit ".$limit;
        ...
        $tipos = DB::select($sql);
		 ...
		 // Paginado
		 $fichasContador = DB::select($sqlContador);
		 $contadorFichas = $fichasContador[0]->total;
		 $cantidadPaginas = ceil($contadorFichas/$registroPorPagina);
		 $contador = (($pagina-1)*$registroPorPagina)+1;
      
		$rol = \Auth::user()->participante->rol_id;
        return view("Modules.Seguimiento.Ficha.index", compact('tipos','rol','fic_numero','contador','cantidadPaginas','contadorFichas','pagina','valor','filtro'));
    ...
    public function getEditar($id) {
		...
		$ficha = DB::select("SELECT * FROM sep_ficha, sep_planeacion_ficha WHERE sep_ficha.fic_numero=sep_planeacion_ficha.fic_numero and sep_ficha.fic_numero= ?", array($id));
    ...
    public function postEditar(Request $request) {
		...
		if ($par_identificacion  != 999999999){
        ...
        DB::update($sql, array($prog_codigo, $cen_codigo, $fic_fecha_inicio, $fic_fecha_fin, $fic_proyecto, $fic_localizacion, $fic_version_matriz, $act_version,$par_identificacionC, $fic_numero));

		$sql2 = "UPDATE sep_planeacion_ficha SET "
                . "pla_ins_lider= ? "
                . "WHERE fic_numero= ?";

        DB::update($sql2, array($par_identificacion, $fic_numero));
		}else{
        $sql = "UPDATE sep_ficha SET "
                . "prog_codigo= ?, "
                . "cen_codigo= ?, "
...
// getVersion
		...
		function getVerdetalle(Request $request) {
			...
			$sql = "select prog.prog_codigo, prog.prog_nombre, fic.fic_numero , fic.fic_fecha_inicio, fic.fic_fecha_fin, par.par_nombres, par.par_apellidos, fic.fic_localizacion, fic.fic_version_matriz from sep_ficha fic, sep_programa prog, sep_participante par, sep_planeacion_ficha pla_fic where fic.prog_codigo = prog.prog_codigo and pla_fic.pla_ins_lider = par.par_identificacion and fic.fic_numero = pla_fic.fic_numero and fic.fic_numero =".$id;			
			$ficha = DB::select($sql);

			if ($ficha == false) {
				$sql = "select sep_ficha.prog_codigo, sep_programa.prog_nombre, sep_ficha.fic_numero, sep_ficha.fic_fecha_inicio, sep_ficha.fic_fecha_fin, sep_ficha.fic_localizacion, sep_ficha.fic_version_matriz from sep_ficha, sep_programa where sep_ficha.prog_codigo = sep_programa.prog_codigo and sep_ficha.fic_numero =".$id;	
				$ficha = DB::select($sql);
	...
	public function getAjaxconsulta(){
		...
		if(is_numeric($vIngresado)){
			...
			FROM 
				...
				sep_nivel_formacion AS niv_for, sep_planeacion_ficha AS pla_fic
			WHERE 
				fic.prog_codigo = pro.prog_codigo 
				AND pla_fic.pla_ins_lider = par.par_identificacion
				AND pla_fic.fic_numero = fic.fic_numero
				AND pro.niv_for_id = niv_for.niv_for_id
